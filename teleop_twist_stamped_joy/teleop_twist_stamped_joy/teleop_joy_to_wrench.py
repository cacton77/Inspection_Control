...
class TeleopJoytoWrench(Node):
    ...
    def joy_callback(self, msg):
        ...
        # Get raw axis values
        fx_raw = msg.axes[self.x_axis] if not self.invert_x else -msg.axes[self.x_axis]
        fy_raw = msg.axes[self.y_axis] if not self.invert_y else -msg.axes[self.y_axis]
        fz_raw_up = (1-msg.axes[self.z_axis_up] )/2.0 # original values were -1 for true and 1 for false so scaled to 1 and 0
        fz_raw_down = (1-msg.axes[self.z_axis_down] )/2.0 # original values were -1 for true and 1 for false so scaled to 1 and 0
        fz_raw = fz_raw_up -fz_raw_down if not self.invert_z else fz_raw_down -fz_raw_up # range from 1 to -1 like x and y axes
        tr_raw_positive = msg.buttons[self.roll_axis_positive]
        tr_raw_negative = msg.buttons[self.roll_axis_negative] 
        tr_raw = tr_raw_positive - tr_raw_negative if not self.invert_z else tr_raw_negative -tr_raw_positive # range from 1 to -1 like x and y axes
        tp_raw = msg.axes[self.pitch_axis] if not self.invert_x else -msg.axes[self.pitch_axis]
        ty_raw = msg.axes[self.yaw_axis] if not self.invert_y else -msg.axes[self.yaw_axis]
